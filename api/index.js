...
module.exports = async (req, res) => {
  ...
  const { contact, channel, source, timestamp, extra } = req.body || {};
  ...
  const channelEmoji = channel === 'Telegram' ? 'TG' : channel === 'WhatsApp' ? 'WA' : 'TEL';
  const time = timestamp ? new Date(timestamp).toLocaleString('ru', { timeZone: 'UTC' }) + ' UTC' : '—';
  const name = extra && extra.name ? extra.name : '—';
  const country = extra && extra.country ? extra.country : '—';

  // Telegram — без Markdown, простой текст
  let telegramOk = false;
  try {
    const text = '🔔 Новая заявка app2.cash\n\n'
      + 'Имя: ' + name + '\n'
      + 'Страна: ' + country + '\n'
      + 'Канал: ' + channelEmoji + ' ' + channel + '\n'
      + 'Контакт: ' + contact + '\n'
      + 'Источник: ' + (source || '—') + '\n'
      + 'Время: ' + time + '\n\n'
    ...
    if (!telegramOk) console.error('TG error:', JSON.stringify(tgRes));
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      html: '<div style="font-family:Arial,sans-serif;max-width:600px">'
        ...
        + '<tr><td style="padding:10px;border-bottom:1px solid #eee;color:#666;width:140px">Имя</td><td style="padding:10px;border-bottom:1px solid #eee;font-weight:bold">' + name + '</td></tr>'
        + '<tr><td style="padding:10px;border-bottom:1px solid #eee;color:#666">Страна</td><td style="padding:10px;border-bottom:1px solid #eee">' + country + '</td></tr>'
        + '<tr><td style="padding:10px;border-bottom:1px solid #eee;color:#666">Канал</td><td style="padding:10px;border-bottom:1px solid #eee;font-weight:bold">' + channelEmoji + ' ' + channel + '</td></tr>'
        ...
        + '</table>'
        + '<div style="background:#fff3cd;border:1px solid #FFD700;border-radius:8px;padding:14px;margin-top:20px;text-align:center"><strong>Свяжитесь в течение 5 минут!</strong></div>'
        + '</div></div>'
    });
    emailOk = true;
